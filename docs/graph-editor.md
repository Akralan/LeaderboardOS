...
- **Bricks and the screen's variables**: `picker` (the instances a lane's field can pick), `stepper` (the same, walked one at a time in order), `form` (a segment of a lane — `segment: 1` for a later call — with some fields fixed by the screen; a segment that draws a claim exposes the claimed item on its variable, a later segment resumes it with `values: {claim_id: $card}`), `frame` and `image` (an application or an image at a chosen URL), `progress` (what I delivered, my counters), and for the `submissions` capability `steps_bar` (the steps, one chosen), `submit_url` (my URL for the chosen step's repositories) and `community` (the others' URLs, kept or not) are the small blocks the hand-written screens decompose into. They coordinate through variables the screen holds: a block that chooses declares `selects: app`, and another block's argument reads `$app`, `$app.app_url`, or `values: {walkthrough: $run, step: $step}` on a form (a form's own `selects` keeps what its gesture created). The validator knows each variable's type from the surface — a variable nobody chooses, a field the chosen instance lacks, a fixed field the segment does not collect are errors. The validator's screen of `journey-validation` is written this way (picker → open form → frame, stepper → record form, complete form) and plays the same routes as the `walkthrough` block; so are the participant's screens of `ml` (steps bar → submit URL and community picks) and `data-annotation` (draw form → image and label form). The one-block components (`walkthrough`, `submissions`, `annotation`…) stay in the catalogue.
