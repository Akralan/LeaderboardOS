...
| `POST` | `/api/templates/author` | `{ description, name?, key? }` — the template author agent writes a draft (generate → validate → repair, at most 3 repairs), creates the template under a free key and saves the draft even with errors. `201 { key, name, diagnostics, choices, openQuestions, rounds, validAfterFirstRound, previewUrl }`; `503` when no OpenAI key is configured. Never publishes. | Admin |
| `POST` | `/api/templates/:key/author/refine` | `{ instruction }` — the agent edits the saved draft by path-addressed edits (untouched text stays byte-identical), validates, repairs and saves. Returns the report and the new `yaml`; `404` without a draft. | Admin |
