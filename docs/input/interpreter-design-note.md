...
**Composed screens** (the UI catalogue). Between the generated stack and a hand-written override there is a middle ground a template can write itself: a `ui` block places components of a closed catalogue (`ui/catalog.ts` — a lane, a text, the participant's work, the board, the workspace, the manager's overview and resources) on a 12-column grid, per screen (`contributor`, `manage`), with typed arguments. The catalogue is the truth on both sides: the validator (`validate/ui.ts`) refuses a component, a screen, an argument or a lane it does not know, and the client mounts the React component of the same name; the surface carries the screens as written (`surface.ui`). A screen left out stays generated. A template still embeds no code — it composes — which is what keeps the escape hatch above (a distribution's hand-written slots) rare rather than necessary.

Composition goes below the screen: the catalogue's *bricks* (`picker`, `stepper`, `form`, `frame`) are the parts a hand-written screen decomposes into, and they coordinate through the screen's **variables** — a brick that chooses declares `selects: app`, another reads `$app.app_url` or fixes a form's fields on `$run` and `$step`. Bindings are checked against the surface like any other reference, so a composition is wired by construction. This is the vocabulary a future authoring agent composes new screens from: declarative, validated, never code executed at a participant's.
