...
- assess:  {id, kind: human | metric | ai_grid | self, fields?: {...}, from?: <collect id>, value?: <expr>, grid?: <expr>,
...
In scope: params.*, counters.*, challenge.state, participation.user, aggregates.<id>.inputs / .verdict,
  every upstream node by id (collect fields: form.field; claim: draw.<type>, draw.substituted; stored observation: probe.<store>; assess: check.value),
...
function catalogText(catalog: CapabilityCatalog): string {
  ...
  return entries
    .map(([name, entry]) => `- ${name} (${entry.kind}) args: ${Object.entries(entry.args).map(([arg, spec]) => `${arg}${spec.required ? "" : "?"}`).join(", ") || "none"}`)
    .join("\n");
}
